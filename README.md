...
This Classic Wordpress theme was created for GNZ's training syllabus website. I first used Gemini 3 Pro to build a wireframe (because I suck at design), then used GPT-5-Codex and Claude Sonnet 4.6 to build this theme and iterate on the UX (with some professional supervision and manual tweaks).
...
### Creating pages
Starting from scratch, you need to create a lot of pages. Please follow this guidance:
...
Regression tests use [Playwright](https://playwright.dev/). Tests run against the local Docker environment (`http://localhost:8080`) and cover both desktop (Chromium) and mobile (iPhone 13 / WebKit viewports).

### Prerequisites

Node.js must be installed, then from the theme directory:
...
# Open Playwright's interactive UI — live browser, step-through, diffs
npm run test:ui
...
### How visual regression workss

The first time `npm test` runs, Playwright writes baseline screenshots to `tests/snapshots/`. On subsequent runs it diffs the current render against those baselines and fails if pixels diverge beyond a small tolerance (0.2%).

When you intentionally change the look of a page, run `npm run test:update-snapshots` to accept the new baseline, then commit the updated `.png` files alongside your code change.

### Adding tests

Tests live in `tests/visual/`. Each file is a standard Playwright test spec. The site runs at `http://localhost:8080`, so make sure Docker Compose is up before running:
